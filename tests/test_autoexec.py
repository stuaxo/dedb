...
def test_autoexec_line_to_userhook_line_passes_an_unrecognised_line_through():
    assert autoexec_line_to_userhook_line("GAME.EXE") == ("GAME.EXE", None)


def test_autoexec_line_to_userhook_line_fast_passes_a_blank_line():
    assert autoexec_line_to_userhook_line("   ") == ("   ", None)


def test_autoexec_line_to_userhook_line_reports_the_shim_that_matched():
    rewritten, hit = autoexec_line_to_userhook_line("IMGMOUNT E disk.img")
...
def test_overlay_mount_is_matched_before_the_plain_mount_rule():
    _rewritten, hit = autoexec_line_to_userhook_line('MOUNT D "..\\saves" -t overlay')
...
def test_autoexec_line_to_userhook_line_defaults_a_missing_working_dir_to_cwd(
    monkeypatch, tmp_path: Path
):
    monkeypatch.chdir(tmp_path)

    rewritten, _hit = autoexec_line_to_userhook_line("MOUNT D SAVES")
...
# --- autoexec_as_userhook (the whole autoexec) ---------------------


def test_autoexec_as_userhook_without_working_dir_comments_out_every_mount(launcher_autoexec_lines):
    result = autoexec_as_userhook(launcher_autoexec_lines)
...
def test_autoexec_as_userhook_leaves_choice_untouched(launcher_autoexec_lines):
    result = autoexec_as_userhook(launcher_autoexec_lines)

    assert "CHOICE /C123 /S Which program do you want to run?: /N" in result


def test_autoexec_as_userhook_preserves_command_order(launcher_autoexec_lines):
    result = autoexec_as_userhook(launcher_autoexec_lines)
...
def test_autoexec_as_userhook_with_working_dir_still_drops_c_and_overlay_mounts(
    tmp_path: Path, launcher_autoexec_lines
):
    result = autoexec_as_userhook(launcher_autoexec_lines, working_dir=tmp_path)
...
def test_autoexec_as_userhook_rewrites_a_secondary_mount_with_a_working_dir(tmp_path: Path):
    (line,) = autoexec_as_userhook(["MOUNT D SAVES"], working_dir=tmp_path)

    assert line == f"LREDIR -f D: {(tmp_path / 'SAVES').resolve()}"


def test_autoexec_as_userhook_comments_out_imgmount(launcher_autoexec_lines):
    result = autoexec_as_userhook([*launcher_autoexec_lines, "IMGMOUNT E disk.img -t iso"])
