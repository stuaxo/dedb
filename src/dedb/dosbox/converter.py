...
def convert(
    ...
    *,
    ...
    output_dir. dosemu_filename/userhook_filename let a caller write more
    than one converted pair into the same output_dir (e.g. one per GOG
    launch profile - see dedb.gog.profiles)."""
    ...
    with userhook_path.open("w", encoding="cp437") as f:
        for command in autoexec_shims(autoexec_commands):
            f.write(f"{command}\n")
